...
//   const saveCapacity = async (ev) => {
//     try {
//       const form = capForm[ev.id] || {
...
//         cap: form.cap
//       })
//       setMessage('Capacity settings saved')
//       await load()
//     } catch (e) {
//       setMessage('Failed to save capacity settings')
//     }
//   }
...
//     }
//     try {
//       const res = await api.post(`/api/registration/event/${eventId}/send-attendance`, { recipients: raw })
//       const cnt = res?.data?.count
//       setPopup({ 
//         open: true, 
//         text: `Attendance sent to ${raw.length} recipient(s).${typeof cnt==='number' ? ` Checked-in rows: ${cnt}.` : ''}` 
...
//       {/* Per-Admin Statistics for Super Admin */}
//       {isSuper && perAdminStats.length > 0 && (
//         <section className="events-section" style={{marginTop: 16}}>
//           <div className="section-header">
//             <h2>Per-Admin Statistics</h2>
...
//                     <div className="event-actions">
//                       <button 
//                         className={`toggle-regs-btn ${Number(ev.regClosed || 0) === 1 ? 'closed' : 'open'}`}
//                         onClick={() => toggleRegistration(ev)}
//                         title={Number(ev.regClosed || 0) === 1 ? 'Open registrations' : 'Close registrations'}
//                       >
//                         {Number(ev.regClosed || 0) === 1 ? 'Open Reg' : 'Close Reg'}
...
//                           onClick={() => saveCapacity(ev)}
//                           title="Save capacity settings"
//                         >
//                           <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
...
//         .toggle-regs-btn.closed {
//           background: rgba(239, 68, 68, 0.1);
//           color: #ef4444;
//           border-color: rgba(239, 68, 68, 0.3);
//         }

//         .toggle-regs-btn.open {
//           background: rgba(34, 197, 94, 0.1);
//           color: #22c55e;
...
export default function AdminDashboard() {
  ...
  const downloadQRCode = async (eventId, eventName) => {
    try {
      const response = await api.get(`/api/events/${eventId}/qr`, {
        responseType: 'blob'
      })
      ...
      link.href = url
      link.setAttribute('download', `${eventName.replace(/\s+/g, '_')}_qr.png`)
  ...
  const getRegistrationLink = (eventId) => {
                  ...
                  <div className="event-header">
                    ...
                    <div className="event-actions">
                      ...
                      <button 
                        ...
                        onClick={() => downloadQRCode(ev.id, ev.name)}
                      ...
                      <button 
                        ...
                        onClick={() => copyRegistrationLink(ev.id)}
                        title="Copy Registration Link"
                      >
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
                          <path d="M8 16C8 18.2091 9.79086 20 12 20H16C18.2091 20 20 18.2091 20 16V12C20 9.79086 18.2091 8 16 8H12C9.79086 8 8 9.79086 8 12V16Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                          <path d="M16 8V6C16 3.79086 14.2091 2 12 2H8C5.79086 2 4 3.79086 4 6V8" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                        </svg>
                        <span>Copy Link</span>
                      </button>
