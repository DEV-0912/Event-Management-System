...
export default function Home() {
    ...
    const loadAllData = async () => {
      ...
      try {
        ...
        const [regsRes, overviewRes] = await Promise.all([
          api.get('/api/registration/mine').catch(() => ({ data: [] })),
          isAdmin ? api.get('/api/events/overview').catch(() => ({ data: { totalEvents: 0, totalRegistrations: 0, totalCheckedIn: 0 } })) : Promise.resolve({ data: { totalEvents: 0, totalRegistrations: 0, totalCheckedIn: 0 } })
        ])
        
        setItems(Array.isArray(regsRes.data) ? regsRes.data : [])
        setOverview(overviewRes.data || { totalEvents: 0, totalRegistrations: 0, totalCheckedIn: 0 })
      } catch (error) {
  ...
  // Always fetch public events so the Upcoming Events section works for everyone
  useEffect(() => {
    let cancelled = false
    ...
    <div className="container">
        ...
        <div className="hero-content">
          ...
          <div className="hero-images">
            <div className="image-grid">
              ...
              />
              <img 
                src="https://images.unsplash.com/photo-1472653816316-3ad6f10a6592?q=80&w=600&auto=format&fit=crop" 
                alt="Speaker on stage" 
                className="hero-image"
                loading="lazy"
              />
      ...
      <section className="upcoming-section">
        ...
        {(!eventsLoading && events.length === 0) ? (
          ...
          <div className="events-scroll">
            {events.map((ev, idx) => (
              <div key={ev.id} className="event-card-wide" style={{ position: 'relative' }}>
                <Link to={`/events/${ev.id}/register`} className="event-card-link" style={{ textDecoration: 'none', color: 'inherit', display: 'block' }}>
                  <div className="event-poster" style={{ position: 'relative', width: '100%', overflow: 'hidden', borderRadius: 12, border: '1px solid var(--border)', cursor: 'pointer' }}>
                    <span style={{ position: 'absolute', top: 8, left: 8, background: 'rgba(0,0,0,0.65)', color: '#fff', fontSize: 12, padding: '4px 8px', borderRadius: 8, zIndex: 2 }}>#{idx + 1}</span>
                    <img 
